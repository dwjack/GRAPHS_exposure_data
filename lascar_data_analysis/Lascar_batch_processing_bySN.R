...
#"~/Dropbox/Ghana_exposure_data_SHARED (1)/CO_files_processed/CO_parameters_all_11894sessions_Jun02.rds" # includes the validation information
...
lascar.import <- function(file,cf, cf_conf) { 
  ...
  cfgood <- !is.na(dt$cf[1]) & dt$cf[1] >=0.2 # THIS IS WRONG, SHOULD BE 0.6-1.2
...
for (i in 1:length(unique(Lascar_data$SN))) { 
  ...
  CO_stacked$lascar <- gsub("\\.", "_", CO_stacked$lascar)
    ...
    for (j in 1:length(unique(CO_stacked$file))) {
      ...
      # evaluate parameters
      cfgood <- !is.na(CO_bysession$cf[1]) & CO_bysession$cf[1] >=0.2 # THIS IS WRONG, SHOULD BE 0.6-1.2
