#
# Run the Shiny application by clicking 'Run App' above.
#
...
shinyServer(function(input, output) {
  ...
  Data <- reactive({
    ...
    if(input$hepasessions >=1) {
      HEPA1_start <- as.POSIXct(paste(input$HEPA1_date, substr(input$HEPA1_times, 1, 5)), origin = "1970-1-1", tz = timezone)
      HEPA1_end <- as.POSIXct(paste(input$HEPA1_date, substr(input$HEPA1_times, 7, 11)), origin = "1970-1-1", tz = timezone)
      HEPATIMES[1,1] <- as.character(paste0(HEPA1_start, "/", HEPA1_end))
    } 
    
    
    if(input$hepasessions >=2) {
      HEPA2_start <- as.POSIXct(paste(input$HEPA2_date, substr(input$HEPA2_times, 1, 5)), origin = "1970-1-1", tz = timezone)
      HEPA2_end <- as.POSIXct(paste(input$HEPA2_date, substr(input$HEPA2_times, 7, 11)), origin = "1970-1-1", tz = timezone)
      HEPATIMES[2,1] <- as.character(paste0(HEPA2_start, "/", HEPA2_end)) 
    } 
    
    
    # testdata: /Users/quinnak/Desktop/HAPIN Guatemala trip/G1_ECM00061_017_16062017_.csv
    #### LOADING DATA ######
    ...
    data <- read.csv(inFile$datapath ,col.names = c("Date","Time",  "RH.Corrected Nephelometer"  , "RH-Corrected Nephelometer HR", "Temp",  "RH",  "Battery",  "Inlet.Press",  "Flow.Orifice.Press",  "Flow",  "X.axis",  "Y.axis",	"Z.axis",	"Vector.Sum.Composite", "ShutDownReason", "Wearing Compliance","ValidityWearingCompliance validation", "V1", "V2", "V3", "V4", "V5"	), header=F, sep=",", fill=T, stringsAsFactors=FALSE) # IMPORTING ACTUAL DATASET INTO R. 
    
    ######### EXTRACTING STOP DESCRIPTIONS/ CREATING TIMESTAMPS/ ESTABLISHING ID FOR FILENAMES ### 
    
    data_header <- data[1:23,1:7]
    colnames(data_header) <- c("variable", "V1", "V2", "V3", "V4", "V5", "V6")
    
    serialnumber <- as.character(data_header[5,2])
    
    data <- data[29:nrow(data),]  
    
    stop.desc=data$ShutDownReason
    ...
    data.withoutdesc <- data[,1:17]
    
    ######### CREATING DATA2 DATASET - ALL DATA INCLUDING HEPA SESSIONS, RH CORRECTED. #####
    
    data2 = data.frame(sapply(data.withoutdesc, blank2na))
    for(i in 1:15){data2[,i+2] = as.numeric(levels(data2[,i+2]))[as.integer(data2[,i+2])]}
    
    data2$Date <- as.character(data2$Date)
    data2$Time <- as.character(data2$Time)
    data2$datetime <- paste(data2$Date, data2$Time)
    
    if (dateformat == "mdy") data2$datetime <- mdy_hms(data2$datetime, tz = timezone)
    if (dateformat == "dmy") data2$datetime <- dmy_hms(data2$datetime, tz = timezone)
    ...
    active.data_summary = matrix(c(round(total_time), 
                                   ...
                                   round(total_minutes_worn/60), # hrs compliant
                                   round((total_minutes_worn/60)/total_time, digits = 3)*100, ## FIXED
